...
var generateImg = function (idMp, action)
{
	...
	newImg.addEventListener('click', function()
	{
		if (confirm('Supprimer ce MP ?'))
		{
			...
			var arguments = 'action_reaction=valid_eff_prive&topic1=' + idMp;
...
if (tr.length > 0) tr.pop().firstChild.setAttribute('colspan', 12);
...
getElementByXpath('//table//tr[starts-with(@class, "sujet ligne_booleen")]', root).forEach(function(tr)
{
	...
	var newImg = generateImg(getElementByXpath('.//td//input[@type="checkbox"]', tr).pop().value, function ()
	{
...
{
	div = div.pop();
	var newImg = generateImg(getElementByXpath('//table//tr[@class="cBackHeader fondForum2Fonctions"]//form//input[@name="post"]', root).pop().value, function ()
	{

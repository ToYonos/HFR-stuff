...
if ($('content_form'))
{
	...
	var searchSmilies = function (event, textAreaId, targetId, changeDisplay)
	{
		...
		if (ctrl == 2)
		{
			...
			if (ss.style.display != "block")
			{
				...
				ss.style.left = ((cw / 2) - 50 + c.offsetLeft) + "px";
				ss.style.top = ((ch / 2) - 12 + c.offsetTop) + "px";
	...
	var pinIt = function (pinImg, pinOn)
	{
		...
		if ($('smilies_helper')) $('smilies_helper').style.display = 'none';
		zone.style.opacity = pinOn ? '1' : '0.25';
	...
	{
		pinIt(this.firstChild, zone.className.indexOf('zoneRepFlot') != -1);
	}
	, false);
	...
	$('content_form').addEventListener('focus', function()
	{
		if (zone.className.indexOf('zoneRepFlot') != -1)
		{
	...
	$('content_form').addEventListener('blur', function()
	{
		if (zone.className.indexOf('zoneRepFlot') != -1)
		{

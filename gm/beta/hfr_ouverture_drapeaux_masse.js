...
({
	...
	setImgUrl : function()
	{
		var imgUrl = prompt("Nombre de drapeaux à ouvrir dans des onglets ?", this.imgUrl);
		if (!imgUrl) imgUrl = '';
	...
	launch : function ()
	{
		...
		{		
			...
			{
				if (self.getElementByXpath('.//td[@class="sujetCase1"]//img[@alt="On"]', tr).length == 1)
				{
					newMps.push(self.getElementByXpath('.//td[@class="sujetCase9"]//a', tr).pop().href);
				}
			}
			);

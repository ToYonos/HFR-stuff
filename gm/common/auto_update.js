({
	check4Update : function()
	{
		...
		var currentVersion = GM_getValue('currentVersion', '##VERSION##');
		// On met éventuellement la version stockée à jour avec la version courante, si la version courante est plus récente
		if (autoUpdate.isLater('##VERSION##', currentVersion))
		{
			GM_setValue('currentVersion', '##VERSION##');
		...
		else if (autoUpdate.isLater(currentVersion, '##VERSION##'))
		{
			GM_registerMenuCommand("##NAME## -> Installer la version " + currentVersion, function()
			{
				GM_openInTab(mirrorUrl + '##PATH####ID##.user.js');
			}
			);
		}
		...
		{
			var checkUrl = mirrorUrl + 'getLastVersion.php5?name=' + encodeURIComponent('##NAME##');
			...
			GM_xmlhttpRequest({
				...
				onload: function(response)
				{
					...
					{
						if (confirm('Une nouvelle version de ##NAME## est disponible : ' + lastVersion + '\nVoulez-vous l\'installer ?'))
						{
							GM_openInTab(mirrorUrl + '##PATH####ID##.user.js');
						}
						else
